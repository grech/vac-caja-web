...
describe("operation backend payloads", () => {
  it("maps browser accountId to the canonical earn locator with purchase amount", () => {
    expect(buildEarnBackendPayload({ businessId: "business", accountId: "account", purchaseAmount: 250 })).toEqual({
  ...
  it("omits purchase amount and calculated deltas for stamps", () => {
    const payload = buildEarnBackendPayload({ businessId: "business", accountId: "account" });
    expect(payload).toEqual({ business_id: "business", customer_loyalty_account_id: "account" });
  ...
  it("maps redeem identifiers and an explicit null branch", () => {
    expect(buildRedeemBackendPayload({ businessId: "business", accountId: "account", rewardId: "reward" })).toEqual({
      business_id: "business",
      account_id: "account",
      reward_id: "reward",
      branch_id: null,
